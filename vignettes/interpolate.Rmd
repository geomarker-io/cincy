...
d_neigh_pop <- 
  d_tract |>
  cincy::interpolate(to = cincy::neigh_cchmc, weights = "pop") |>
  st_transform(3735)
```
...
d_neigh_homes <- 
  d_tract |>
  cincy::interpolate(to = cincy::neigh_cchmc, weights = "homes") |>
  st_transform(3735)

d_neigh_area <- 
  d_tract |>
  cincy::interpolate(to = cincy::neigh_cchmc, weights = "area") |>
  st_transform(3735)
```
...
d_neigh_long <- left_join(cincy::neigh_cchmc, d_neigh_long, by = "neighborhood") |>
  st_transform(3735) |>
